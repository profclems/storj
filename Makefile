...
push-modular-storagenode-image: ## Push modular storagenode Docker image
	./storagenode/storagenode/docker.sh push --repo ghcr.io/storj/storagenode-modular --latest --unified-tag

.PHONY: darwin-binaries
darwin-binaries:
	echo "Building Darwin binaries (identity, uplink) for amd64, arm64"
	@components="identity uplink"; \
	arches="amd64 arm64"; \./storagenode/storagenode/docker.sh push --arch amd64,arm64v8,arm32v5
